...
// 0: root MemoryAuthority   1: InterruptAuthority   2: DeviceAuthority   3: discovery Region   4: boot-module Region
// 5: DmaAuthority

/// Build the first AddressSpace, raw-map Flint from the initrd, pre-load its HandleTable with the capability bundle,
/// and start its first thread. `arg` carries the discovery blob's byte offset into its page-aligned Region.
pub fn start(initrd: machine_module.MemoryRange, discovery: PhysAddr, discovery_length: usize) Error!void {
    ...
    // The capability bundle (04-boot-and-bootstrap.md): all authority plus discovery and boot module, nothing ambient.
    ...
    const discovery_page = std.mem.alignBackward(PhysAddr, discovery, page_size);
    const discovery_top = std.mem.alignForward(PhysAddr, discovery + discovery_length, page_size);
    const discovery_region = try Region.wrap(discovery_page, discovery_top - discovery_page);
    ...
    const grants = [_]process_module.Grant{
        ...
        .{ .object = &discovery_region.header },
